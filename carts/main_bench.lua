...
local _cpu=setmetatable({},autozero_cls)
...
function play_state()
  ...
  return 
    ...
    function()
      ...
      local y,total=0,stat(1)
      
      for n,cpu in pairs(_cpu) do
        local avg=_avg[n] or {}
        add(avg,cpu)
        if(#avg>30) deli(avg,1)
        _avg[n]=avg
      end
      for n,avg in pairs(_avg) do
        local sum=0
        for _,v in pairs(avg) do
          sum+=v
        end
        print(n..":"..flr((100*sum/#avg)/total),2,y,12)
        y+=6
      end
      -- spr(0,0,64,16,8)
...
function _update()
  -- reset perf counter
  _cpu=setmetatable({},autozero_cls)
...
function unpack_actors()
  ...
  local function_factory={
    ...
    function()
      ...
      return profile("A_FireBullets",function(owner)
        -- find 'real' owner
    ...
    function()
      ...
      return profile("A_FireProjectile",function(owner)
        -- find 'real' owner
        owner=owner.owner or owner
    ...
    function()
      ...
      return profile("A_FaceTarget",function(self)
        -- nothing to face to?
        if self.target then
    ...
    -- A_Look
    function()
      return profile("A_Look",function(self)
    ...
    function(item)
      ...
      return profile("A_Chase",function(self)
        -- still active target?
...
::loop::
              ...
              -- get ticks
              ticks=state[1]
              -- trigger action function (if any)
