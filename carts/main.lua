      ...
      if is_player then
        intersect_sub_sector(ss,self,{cos(self.angle),-sin(self.angle)},0,radius+48,0,function(hit)
          ...
          if ldef.trigger and ldef.playeruse then
            -- use special?
            if btnp(_btnuse) or mbtnp(2) then
              ldef.trigger(self)
            end
    ...
    tick=function(self)
      ...
      if not self.dead then
        ...
        if _wp_hud then
          ...
          if(_slow==0) _wp_hud=not (btn(6) or mbtnp(4))
  ...
  return
    ...
    function()
      ...
      if idle_ttl<0 then
        ...
        elseif btnp(_btnfire) or mbtnp(1) then
          -- 3: retry
  ...
  local function_factory={
    ...
    function(item)
      return function(owner,weapon)
        if not _wp_hud and (btn(_btnfire) or mbtn(1)) then
