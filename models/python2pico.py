...
def to_multicart(s,pico_path,carts_path,cart_name,boot_code=(),label=None):
  cart_id = 0
  cart_data = ""
  for b in tqdm(s, desc="Generating carts", unit="bytes"):
    ...
    if len(cart_data)==2*0x4300:
        to_cart(cart_data, pico_path, carts_path, cart_name, cart_id, cart_id<len(boot_code) and boot_code[cart_id], label=cart_id==0 and label)
        cart_id += 1
        cart_data = ""
  # remaining data?
  if len(cart_data)!=0:
